...
public class AuxiliarService implements Serializable {
	 ...
	 */
	public EnderecoVO recuperarEndereco(Integer cep) {
		Gson gson = new Gson();
		String json = jsonBuilder("http://cep.paicon.com.br/json/" + cep);
		return gson.fromJson(json, new TypeToken<EnderecoVO>() {
		}.getType());
	}
...
//	}
	...
	public Double calcularDistanciaKm(Cidade cidadeUsuarioGrid, Cidade cidadeUsuarioSessao) {
		
//		Cidade cidadeUsuarioGrid = serviceContato.recuperarContatoUsuario(usuarioGrid).getCidade();
//		Cidade cidadeUsuarioSessao = serviceContato.recuperarContatoUsuario(usuarioSessao).getCidade();
		
		double latitudeP1 = cidadeUsuarioGrid.getLatitude();
		double longitudeP1 = cidadeUsuarioGrid.getLongitude();
		double latitudeP2 = cidadeUsuarioSessao.getLatitude();
		double longitudeP2 = cidadeUsuarioSessao.getLongitude();
	    ...
	    return RAIO_TERRA * 2 * Math.asin(Math.sqrt(valorHaversine));
